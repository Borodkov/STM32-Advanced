...
// buffer in SRAM
#define COLOR_CONVERSION_0 GUICC_M565//GUICC_M8888I//GUICC_M565
// buffer in SRAM
...
/**
  ...
  * @param  hLtdcHandler: pointer to a LTDC_HandleTypeDef structure that contains
  *                the configuration information for the specified LTDC.
  * @retval None
  */
void HAL_LTDC_LineEvenCallback(LTDC_HandleTypeDef *hLtdcHandler) {
  uint32_t Addr;
  uint32_t layer;
  // this function is used by BSP for display Layer 1 only
  InterruptCheckAndSwitchDoubleBuffer();
  ...
  for (layer = 0; layer < GUI_NUM_LAYERS; layer++) {
    if (layer_prop[layer].pending_buffer >= 0) {
      ...
      HAL_LTDC_SetAddress(hLtdcHandler, Addr, layer);
...
int LCD_X_DisplayDriver(unsigned LayerIndex, unsigned Cmd, void * pData) {
  ...
  switch (Cmd) {
    ...
    case LCD_X_SETORG:
      ...
      HAL_LTDC_SetAddress(&hLtdcHandler, addr, LayerIndex);
      break;
    ...
    case LCD_X_SETLUTENTRY:
      HAL_LTDC_ConfigCLUT(&hLtdcHandler, (uint32_t *) & (((LCD_X_SETLUTENTRY_INFO *)pData)->Color), 1, LayerIndex);
      break;
    ...
    case LCD_X_SETPOS:
      HAL_LTDC_SetWindowPosition(&hLtdcHandler,
    ...
    case LCD_X_SETALPHA:
      HAL_LTDC_SetAlpha(&hLtdcHandler, ((LCD_X_SETALPHA_INFO *)pData)->Alpha, LayerIndex);
      break;
    ...
    case LCD_X_SETCHROMA:
      ...
      HAL_LTDC_ConfigColorKeying(&hLtdcHandler, Color, LayerIndex);
      break;
  ...
  if (LayerIndex < GUI_NUM_LAYERS) {
    ...
    if (LCD_GetBitsPerPixelEx(LayerIndex) <= 8) {
      ...
      if (layer_prop[LayerIndex].pColorConvAPI == GUICC_88666I) {
        ...
        HAL_LTDC_ConfigCLUT(&hLtdcHandler, LUT, 256, LayerIndex);
      }
    }
  }
}
