...
/*  scale the pDataIn array
    ...
    arg uint32_t inMax - input maximum valeu
...
void ScopeScale(short *pDataIn, short *pDataOut, uint32_t inMax, uint32_t outMax, uint32_t numSamples) {
    ...
    for (i = 0; i < numSamples; i++) {
        /*scale values*/
        pDataOut[i] = ((((int32_t)pDataIn[i]) * outMax) / inMax);
    }
}

///*scale the pDataIn array horizontaly
//*arg short * pDataIn - input array for scale
//*arg short * pDataOut - output scalled array
//*arg uint32_t maxSamples - input maximum valeu
//*arg uint32_t numSamples - array widtd
//*/
//void ScopeHorizontalScale(short * pDataIn, short * pDataOut, uint32_t maxSamples, uint32_t numSamples)
//{
//	uint32_t i;
//	for(i=0;i<numSamples;i++)
//	{
...
void ScopeLCD_Show(short *pData) {
    ...
    if ((oscConfig.numberOfSamples) /*- triggerSample*/ > LCD_GetXSize() - GRAPH_BORDER_L) {
        numToShow = LCD_GetXSize() - GRAPH_BORDER_L;
    } else
        /*calculate how many samples application can show*/
    {
        numToShow = (oscConfig.numberOfSamples) /*- triggerSample*/;
    }

    /*scale pData buffer*/
    ScopeScale(&pData[0], &pData[0], 4224, LCD_GetYSize(), numToShow);		// scale the data amplitude for show
    /*Plot Y axis*/
    hData  = GRAPH_DATA_YT_Create(SCOPE_STBLUE, numToShow, &pData[0], numToShow);
...
void ScopeLCD_Init(void) {
    ...
    // scale to full ADC resolution
    GRAPH_SCALE_SetFactor(hScale, (float)1 / (float)LCD_GetYSize() * (float)3.4);
#endif
}
